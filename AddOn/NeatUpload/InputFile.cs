 ...
 *
 *  Notes:
 *      $Id: InputFile.aspx.cs 170366 2010-09-08 05:34:25Z ACHIEVO\daly.zeng $.
 *  Revision History
 *  &lt;Date&gt;,    &lt;Who&gt;,    &lt;What&gt;
 * </pre>
 */
...
{	
	...
	public class InputFile : System.Web.UI.WebControls.WebControl, System.Web.UI.IPostBackDataHandler
	{
		...
		*/
        ...
        public Hashtable MultiFiles
        {
            get
            {
                ...
                if (Config.Current.UseHttpModule)
                {
                    ...
                    {
                        return UploadContext.Current.uploadedFiles;
                    }
                }
        ...
        /// <summary>
        /// Create UploadFile by HttpPostedFile
        /// </summary>
        /// <param name="postedFile">HttpPostedFile control</param>
        /// <returns></returns>
        public UploadedFile CreateUploadedFile(HttpPostedFile postedFile)
        {
            ...
            if (postedFile != null)
            {
                ...
                ctx.Status = UploadStatus.NormalInProgress;
                UploadStorageConfig storageConfig = UploadStorage.CreateUploadStorageConfig();
                string storageConfigString = this.Page.Request.Form[UploadContext.ConfigNamePrefix + "-" + postedFile.FileName];
                if (storageConfigString != null && storageConfigString != string.Empty)
                {
                    storageConfig.Unprotect(storageConfigString);
                }
                uploadFile = UploadStorage.CreateUploadedFile(ctx, postedFile.FileName, postedFile.FileName, postedFile.ContentType, storageConfig);
                Stream outStream = null, inStream = null;
                try
                {
                    outStream = uploadFile.CreateStream();
                    inStream = postedFile.InputStream;
                    byte[] buf = new byte[4096];
                    int bytesRead = -1;
                    while (outStream.CanWrite && inStream.CanRead
