 ...
 *  File: FilteringWorkerRequest.aspx.cs
 *
 *  Accela, Inc.
 *  Copyright (C): 2010
 *
 ...
 *
 *  Notes:
 *      $Id: FilteringWorkerRequest.aspx.cs 170366 2010-09-08 05:34:25Z ACHIEVO\daly.zeng $.
 *  Revision History
 *  &lt;Date&gt;,    &lt;Who&gt;,    &lt;What&gt;
 * </pre>
 */
...
*/
            ...
            if (this.grandTotalBytesRead > UploadHttpModule.MaxRequestLength * fileCount)
			{
				if (log.IsDebugEnabled) log.Debug("Request Entity Too Large");
				throw new UploadTooLargeException(UploadHttpModule.MaxRequestLength * fileCount);
			}
		...
		internal void ParseMultipart()
		{
			...
			catch (Exception ex)
			{
				...
				try
				{
                    ...
                    HttpContext.Current.ApplicationInstance.CompleteRequest(); //add back -james.shi
				}
				catch (Exception)
				{
		...
		{			
			...
			while (CopyUntilBoundary())
			{
				...
				while (null != (header = GetLine()))
				{
                    ...
                    // get file count
                    if (name != null && name.IndexOf("hfNeatUploadFileCount") != -1)
                    {
                        //fileCount = 0;
                        GetLine();  // Blank
                        string num = GetLine(); // get the value
                        parsePos = parsePos - num.Length;   // back to prior for CopyUntilBoundary operation

                        if (int.TryParse(num, out fileCount) == false)
                        {
                            fileCount = 1;
                        }
                        break;
				         ...
				         && null != (controlID = translator.FileFieldNameToControlID(name)))
				{
                    ...
                    if (origContentLength > UploadHttpModule.MaxRequestLength * fileCount)
					{
                ...
                {
                    UploadedFile uploadedFile = uploadContext.CreateUploadedFile(name, fileName, contentType, null);
                    outputStream = fileStream = uploadedFile.CreateStream();
                    readPos = parsePos; // Skip past the boundary and headers


                    // Write out a replacement part that just contains the filename as the value.
                    WriteReplacementFormField(name, fileName);
                }
				else
				{
					outputStream = preloadedEntityBodyStream;
